...
def rule_matches(rules, memo, index):
    ...
    if rule[0] == 0:
        ...
        for group in rule[1]:
            ...
            res = rule_matches(rules, memo, indices[0])
            for i in indices[1:]:
                parts = rule_matches(rules, memo, i)
...
def part1(rules, memo, messages):
    ok = rule_matches(rules, memo, 0)
    return sum([1 for m in messages if m in ok])


def part2(rules, memo, messages):
    prefixes = rule_matches(rules, memo, 42)
    suffixes = rule_matches(rules, memo, 31)
